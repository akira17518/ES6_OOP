...
document.querySelector(".modal-body #form").onchange = () => {
    ...
    if (userEnum == 1) {
        ...
        // Reset các lỗi
        rowNhanVienThongBao.forEach(item => reset(item.getAttribute("id")));
        rowKhachHangThongBao.forEach(item => reset(item.getAttribute("id")));
        ...
        // Reset các lỗi
        rowSinhVienThongBao.forEach(item => reset(item.getAttribute("id")));
        rowKhachHangThongBao.forEach(item => reset(item.getAttribute("id")));
    } else if (userEnum == 3) {
        document.querySelector(".sinhVienInputContainer").style.display = "none";
        document.querySelector(".nhanVienInputContainer").style.display = "none";
