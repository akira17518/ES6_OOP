...
document.querySelector(".modal-body #form").onchange = () => {
    ...
    if (userEnum == 1) {
        ...
        // Reset các lỗi
        rowNhanVienThongBao.forEach(item => reset(item.getAttribute("class")));
        rowKhachHangThongBao.forEach(item => reset(item.getAttribute("class")));
        ...
        // Reset các lỗi
        rowSinhVienThongBao.forEach(item => reset(item.getAttribute("class")));
        rowKhachHangThongBao.forEach(item => reset(item.getAttribute("class")));
    } else if (userEnum == 3) {
        document.querySelector(".sinhVienInputContainer").style.display = "none";
        document.querySelector(".nhanVienInputContainer").style.display = "none";
