...
let formList = document.querySelectorAll(".modal-body #form");
formList.forEach(form => form.onchange = () => {
    console.log(form);
...
document.querySelectorAll("#btnDong").forEach(closeButton => closeButton.addEventListener("click", function () {
...
window.updateUser = id => {
    let userUpdate = userList.getIdUpdate(id);
    if (userUpdate) {
        let arrayInput = document.querySelectorAll(".personInputContainer input", ".personInputContainer select", ".nhanVienInputContainer input", ".khachHangInputContainer input");
